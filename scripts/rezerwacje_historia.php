...
$rezultat = mysql_query ( $kwerenda, $db_link );
 ...
 $ile_wierszy = mysql_num_rows( $rezultat ); 
  ...
  echo('<td width="25%" bgcolor="#82B8FB" align="center"><b>Data złożenia rezerwacji </b></td>');
	   ...
	   $rezultat3 = mysql_query ( $kwerenda3, $db_link );
		...
		$marka = $wynik3[0];
		 ...
		 echo('<tr style="background-color: white; height: 26px;">');
	      ...
	      echo('<form name="anuluj_rez" method="post" action="./index.php" onsubmit="return  confirm(\'Czy na pewno chcesz odwołać rezerwację?\')" >');
	
		  echo("<input type=\"hidden\" name=\"anuluj_rez\" value=\"$nr_rezerwacji\" />");
		  echo("<input type=\"hidden\" name=\"anuluj_rauto\" value=\"$auto_id\" />");
		  echo("<input type=\"hidden\" name=\"anuluj_rosoba\" value=\"$id_uzytkownika\" />"); 			  
		  
	 	  echo('<input type="submit" value="Anuluj rezerwację" class="przycisk2" />');
		  echo("</form>");
		  echo("</td>");
		 echo("</tr>");
		 echo("</table>");
